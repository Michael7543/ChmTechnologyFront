...
export class AuthInterceptor implements HttpInterceptor {
  ...
  intercept(
    ...
    const cacheBustingRequest = request.clone({
      setParams: { _t: new Date().getTime().toString() },
    });

    return this.handleRequestWithTokenExpirationCheck(
      cacheBustingRequest,
      req,
      next
    );
  ...
  private handleRequestWithTokenExpirationCheck(
  ...
  private handle401Error(
    ...
    return this.authService
      .refreshToken(decryptedUsername, decryptedPassword, refreshToken)
      .pipe(
        switchMap((refreshResponse) => {
          ...
          const updatedRequest = this.addToken(
            request,
